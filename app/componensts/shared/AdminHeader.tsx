...
const AdminHeader = ({title, description}: TProps) => {
  ...
  // console.log(data?.profile?.profile_picture);
  return (
    <div className='bg-common p-5 rounded-xl flex justify-between items-center border border-border-color-color'>
      ...
      <div className="flex items-center gap-5">
        ...
        <div>
            <h3 className='font-semibold text-xl text-header capitalize'>Richel</h3>
            <p className='font-medium text-title text-[16px] capitalize'>{data?.name}</p>
        </div>
      </div>
    </div>
  )
